...
too and say which is which; the videos show the print file's words and slurs.
...
offset would shift everything; it measured 0.0 ms) and when each light comes on
in the picture against the onsets in the file's audio (lights 15–35 ms early,
about one frame). Report both lines in the handback.
...
- **Keeping one staff while hiding the others** takes an invisible note:
  ...
  `<space>` does not keep the staff. The extra layer pushes a whole-bar rest
  above the staff; `loc="6"` on the `mRest` puts it back.
...
- **Two noteheads on one spot.** A unison between the two voices of a divided
  bar draws two identical noteheads. The lights are pixel-exact because each
  lightable piece is rendered into bit planes of its index; overlapping pieces
  OR their bits into some third piece's index, and on the two-soloist TTBB three
  unlit Baritone notes lit up with another note. Rendering the inverted planes
  too finds every overlapped pixel; noteheads at the same spot are then treated
  as one note lit in both colours. Place halos from the SVG's notehead positions,
  not from pixels, or touching seconds give tall ovals.
- **cairosvg honours `visibility="visible"` inside a hidden parent**, which is
  what makes the bit planes cheap: hide the page, show only the pieces for that
  bit. It ignores `@font-face`, so the metronome-mark note is a box until Leipzig
  is installed (8.4); the script does that itself when fonttools and brotli are
  there.

The script runs where there is time: a desktop bridge that kills background jobs
and stops each command at three minutes cannot render a whole song, so render in
a container and copy the mp4s back (about 14 MB each for 4:48 at 1920×1080).
